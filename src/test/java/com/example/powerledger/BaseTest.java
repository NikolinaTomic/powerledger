...
public class BaseTest {
	...
	protected static List<Battery> createBatteryList() {
		...
		while (i <= listSize) {
			batteries.add(Battery.builder().name("Battery" + i).postcode(10 * i).wattCapacity(10 * i).build());
			i++;
		}
		return batteries;
	}
	...
	protected static List<BatteryDTO> createBatteryDTOList() {
		...
		while (i <= listSize) {
			batteriesDTO.add(BatteryDTO.builder().name("Battery" + i).postcode(10 * i).wattCapacity(10 * i).build());
			i++;
		}
		return batteriesDTO;
	}
